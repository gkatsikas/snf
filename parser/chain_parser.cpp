...
short ChainParser::test_chain_nf(void) {
	...
	for ( Vertex* v : this->chain_graph->get_chain()->get_vertex_order() ) {
		...
		for ( auto& endpoint : nf_graph->get_endpoint_vertices(VertexType::Input) ) {
			std::string name     = endpoint->get_name();
			std::string el_class = endpoint->get_class();

			log << "\tElement  Name: " << name     << def << std::endl;
...
short ChainParser::verify_and_connect_nfs(std::string nf_name, unsigned short position) {
		...
		if ( (el_class == "FromDevice")   || (el_class == "PollDevice") ||
			...
			// This is an internal interface that connects the NF with a subsequent NF
				...
				if ( type == VertexType::Output ) {
					...
					log << "\tElement found is " << next_jump->get_class() << def << std::endl;

					// Update jump pointers for this Output element by passing the pointer
					// of the next Input element just found
					element->add_jump_element(next_jump);
					//nf_graph->add_edge(element, next_jump);
					log << "\tElement's " << name << " jump  pointer is updated" << def << std::endl;
	...
	for ( auto& element : io_elements ) {
		...
		std::vector<std::shared_ptr<ElementVertex>> list = element->get_jump_to_next_nfs();
		...
		for ( auto& neighbour : list ) {
			log << "\t\t Neighbour: " << neighbour.get()->get_name() << def << std::endl;
		}
	}*/
