...
iprw4 :: IPRewriter(pattern 15.0.0.1 1024-65535 - - 1 0, DEC_IP_TTL true, CALC_CHECKSUM true);
iprw4[1] -> Discard ();
iprw4[0] -> Discard ();

/** NAT going to: NF_4,netmap:nf4veth0, BURST 32, METHOD NETMAP **/
iprw3 :: IPRewriter(pattern 14.0.0.1 1024-65535 - - 1 0, DEC_IP_TTL true, CALC_CHECKSUM true);
iprw3[1] -> Discard ();
iprw3[0] -> Discard ();
...
iprw6[1] -> EtherEncap(0x0800, $macAddr1, $gwMACAddr1) -> Queue ($queueSize) -> ToDevice ($iface1, BURST 32, METHOD NETMAP);
iprw6[0] -> Discard ();
...
ipc0 :: IPClassifier (
	...
	(((dst net 15.0.0.0/24) && !((dst net 10.0.0.0/24) || (dst net 11.0.0.0/24) || (dst net 12.0.0.0/24) || (dst net 13.0.0.0/24) || (dst net 14.0.0.0/24) || (dst net 15.0.0.1/32) || (dst net 15.0.0.2/32) || (dst net 200.0.0.1/32)))),
