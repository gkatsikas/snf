...
void ConsolidatedTc::add_tc (const TrafficClass& tc, const TrafficClassFormat& tc_format) {
 ...
 */
short Synthesizer::get_hyper_nf_ifaces_no(void) {
	return this->hyper_nf_ifaces.size();
}

void Synthesizer::print_hyper_nf_ifaces(void) {
 ...
 */
short Synthesizer::generate_equivalent_configuration(bool to_file) {
 ...
 */
short Synthesizer::generate_all_in_soft_configuration(bool to_file) {
...
short Synthesizer::generate_flow_director_split_pipelines(bool to_file) {
	if ( to_file ) {
...
short Synthesizer::generate_openflow_split_pipelines (bool to_file) {
	if ( to_file ) {
		// Open the output files to host our synthesized chain
		//soft_out_file = new std::ofstream(this->soft_configuration_filename);
		// One per nic
 ...
 */
bool Synthesizer::allocate_queues_to_cores(
		std::map <std::string, unsigned short> &nic_desc_to_core,
		std::map < std::string, std::vector<std::string>>& nic_desc_to_ip_class,
 ...
 */
bool Synthesizer::schedule_core_threads_on_queues(std::map <std::string, unsigned short>& nic_desc_to_core, 
		std::string& code_buffer) {
 ...
 */
bool Synthesizer::generate_rss_configuration(std::ofstream **hw_out_file, unsigned short nics_no,
 ...
 */
void TrafficBuilder::traffic_class_builder_dfs(
	Graph* graph,
