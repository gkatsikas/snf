		...
		for (auto &endpoint : nf_graph->get_endpoint_vertices(VertexType::Input)) {
			...
			for (auto &tc : ct.get_traffic_classes()) {
				...
				if (!tc.is_discarded()) {
					...
					if (tc_out_iface.empty()) {
						error_chatter(
							this->log,
							"\t\tTraffic class is created without an output interface."
						);
						return TO_BOOL(CHAIN_SYNTHESIS_PROBLEM);
					}

					std::string snd_key = op_as_str + "\\" + tc_out_iface;
					...
					if (this->tc_per_input_iface[key].find(snd_key) == this->tc_per_input_iface[key].end()) {
						...
						debug_chatter(this->log, "\t\tAdded traffic class towards " << tc_out_iface);
						debug_chatter(this->log, "\t\tTraffic class: " << tc.to_str());
					}
...
{
	if (nf_iface.first.empty() || nf_iface.second.empty()){
		return "";
	}
...
{
	if (nf_iface.first.empty() || nf_iface.second.empty() || nic.empty()) {
		return;
	}
...
TrafficBuilder::retrieve_lb_patterns_from_st_element(ElementVertex *ev)
{
	...
	if (
		(el_class != std::string("IPAddrRewriter"))  || (el_class != std::string("IPAddrPairRewriter")) ||
