...
ParserConfiguration::parse_topology(const std::string &nf_topo) {
	if ( ! this->nf_chain ) {
		this->usage("\tThe graph of the chain does not exist.", 
	...
	for (boost::tokenizer<boost::char_separator<char>>::iterator line=expressions.begin(); line!=expressions.end(); ++line) {
		...
		if ( line->find("->") == std::string::npos ) {
			this->usage("\tEach connection statement must have two parts separated by a pointer (->).", 
		...
		for (boost::tokenizer<boost::char_separator<char>>::iterator elem=connections.begin(); elem!=connections.end (); ++elem) {
			...
			if ( left_bracket == std::string::npos ) {
				this->usage("\tAn interface must be specified in []",
							"\tSyntax: NF_1[iface]->[iface]NF_2;");
				return TO_BOOL(CHAIN_PARSING_PROBLEM);
			}
			...
			if ( position > elements_in_property ) {
					this->usage("\tChain length is " + std::to_string(elements_in_property) 
	...
	if ( elements_in_property > graph_elements ) {
		...
		this->usage("\tThere are " + std::to_string(graph_elements - elements_in_property) + 
...
ParserConfiguration::parse_uni_chain(const std::string &nf_topo) {
	...
	for (boost::tokenizer<boost::char_separator<char>>::iterator out_iface=expressions.begin(); out_iface!=expressions.end(); ++out_iface) {
		...
		int position = atoi(get_number_from_string(nf).c_str());
		if ( position != 1 ) {
			this->usage("\tChain length is 1 but NF_" + std::to_string(position) + " found", 
...
ParserConfiguration::parse_domains(const std::string &nf_domains) {
	...
	for (boost::tokenizer<boost::char_separator<char>>::iterator point=connections.begin(); point!=connections.end(); ++point) {
		...
		if ( !v ) {
			this->usage("\t" + nf + " does not exist in the topology",
